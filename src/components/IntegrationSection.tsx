...
const IntegrationSection = ({ id }: IntegrationSectionProps) => {
  const strategyPlatforms = [
    { name: "TradingView", description: "Pine Script Integration", icon: TrendingUp, color: "bg-gradient-to-br from-blue-500 to-blue-600", hoverColor: "hover:from-blue-600 hover:to-blue-700" },
    { name: "Python", description: "Custom Algorithms", icon: Code, color: "bg-gradient-to-br from-yellow-500 to-yellow-600", hoverColor: "hover:from-yellow-600 hover:to-yellow-700" },
    { name: "Excel", description: "Spreadsheet Trading", icon: Calculator, color: "bg-gradient-to-br from-purple-500 to-purple-600", hoverColor: "hover:from-purple-600 hover:to-purple-700" }
  ];

  const brokers = [
    { name: "TradingView", status: "active", logo: "📈" },
    { name: "CoinDCX", status: "active", logo: "🪙" }
  ];

  const comingSoonBrokers = [
    { name: "Delta Exchange", status: "coming-soon", logo: "🚀" },
            ...
            {strategyPlatforms.map((platform, index) => (
              <div key={platform.name} className="group relative bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                ...
                <div className="relative">
                  <div className={`${platform.color} ${platform.hoverColor} w-14 h-14 rounded-xl flex items-center justify-center mb-4 mx-auto shadow-lg transition-all duration-300 group-hover:scale-110`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {comingSoonBrokers.map((broker) => (
                <div key={broker.name} className="group bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 text-center border border-gray-200 hover:border-orange-200 transform hover:-translate-y-1">
                  <div className="text-3xl mb-3 group-hover:scale-110 transition-transform opacity-60 group-hover:opacity-80">{broker.logo}</div>
